...
const signupUser = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: newUser._id, role: newUser.role, firstname: newUser.firstname, email: newUser.email }, process.env.JWT_SECRET, { expiresIn: "7d" });
...
const loginUser = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user._id, role: user.role, firstname: user.firstname, email: user.email }, process.env.JWT_SECRET, { expiresIn: "7d" });
